...
def insert(people,Surname,contacts_number,Staff_ID):
    conn=sqlite3.connect("contacts.db")
    cur=conn.cursor()
    cur.execute("INSERT INTO contacts VALUES (NULL,?,?,?,?)",(people,Surname,str(contacts_number),Staff_ID))
    conn.commit()
    conn.close()
    search(people=people,Surname=Surname,contacts_number=contacts_number,Staff_ID=Staff_ID)
